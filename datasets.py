...
class VideoSummDataset(object):
    def __init__(self, keys, args=None):
        ...
        text_feature_path = '{}/{}/feature/text_deberta.npy'.format(args.data_root, args.dataset)
        # text_feature_dict = np.load(text_feature_path, allow_pickle=True).item()
        text_feature_dict = np.load(text_feature_path, allow_pickle=True)
        
        # print(text_feature_dict)
        # print(type(text_feature_dict))
        text_feature_dict=text_feature_dict.item()
        # text_feature_dict=text_feature_dict.astype(float32)
        # print(type(text_feature_dict))

        # text_feature_dict = np.load(text_feature_path, allow_pickle=True).todict()

        # text_feature_dict=dict(enumerate(text_feature_dict,1))
